...
\begin{abstract}
  ...
  favor strong levels of central galaxy assembly bias. Our analyses favor models 
  ...
  assembly bias in the $\magr<-21$ sample. In addition to presenting the 
  constraints on HOD models that accommodate assembly bias, our analysis 
  includes numerous improvements over previous analyses of this data set and 
  supersedes existing constraints even on standard HOD parameters.
\end{abstract}
...
\end{equation}
%
For every host halo in the catalog we draw a random number from a uniform distribution $\mathcal{U}(0, 1);$ for a host halo of present-day virial mass $\mvir,$ a central galaxy is assigned to the halo if the associated random number is less than $\langle N_{\mathrm{cen}} \vert \mvir \rangle;$ halos with random values exceeding  $\langle N_{\mathrm{cen}} \vert \mvir \rangle$ are left devoid of centrals.
The parameter $\log (M_{\rm min})$ specifies the halo mass at which the halo has a 50\% probability of hosting a
central galaxy, while the parameter $\sigma_{\log M}$ specifies the rate at which $\langle N_{\mathrm{cen}} \vert \mvir \rangle$
...
%
\begin{eqnarray}
  \delta N_{\rm cen} & = & A_{\rm cen} \, {\rm MIN}\left\{\langle N_{\mathrm{cen}}|\mvir\rangle , 1- \langle N_{\mathrm{cen}}|\mvir\rangle\right\}\nonumber\\
  \delta N_{\rm sat} & = & A_{\rm sat} \, \langle N_{\mathrm{sat}}|\mvir\rangle
\end{eqnarray}
%
...
\begin{equation}
\label{eq:chisquare}
\chi^2 = \sum_{i,j} \Delta w_{{\rm p},i} \, [C^{-1}]_{ij} \, \Delta w_{{\rm p},j} + \frac{(n_{\rm g}^{\rm mock} - n_{\rm g}^{\rm meas})^2}{\sigma_{\rm n}^2},
\end{equation}
%
...
galaxy number density. The error on the galaxy number density assumes Poisson statistics for both the measured and predicted galaxy number densities.
...
all fits have a probability $\gtrsim 1\% $ of obtaining a higher value of $\chi^2$ by chance. An example of the inferred posteriors for the HOD parameters is shown in Figure~\ref{fig:Mr20triangle} for the sample with $\magr < -20$. We do 
...
by examining the top two rows in each luminosity threshold grouping in Table~\ref{table:parameters}. 
...
in {\tt Halotools} \cite{halotools} is discussed further in Section~\ref{subsection:decorated} above 
